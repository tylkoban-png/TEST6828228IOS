            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                    ...
                    style:
                        TextStyle(color: _textSec, fontSize: 14, height: 1.5)),
              ],
            ),
          ),
          const SizedBox(height: 16),
...
class _YouTubeState extends State<_YouTube> with AutomaticKeepAliveClientMixin {
  ...
  static const _jsVisibility = r"""
    (function() {
      ...
      ['visibilitychange','webkitvisibilitychange','blur','pagehide','freeze'].forEach(function(ev) {
  ...
  static const _jsLogic = r"""
    (function() {
      ...
      function attach(v) {
        ...
        v.addEventListener('play', function() { window.__userPaused = false; });
        v.addEventListener('ended', function() { window.__userPaused = false; });
        v.addEventListener('loadstart', function() { window.__userPaused = false; v.__att = false; });
      }

      function getVideo() { return document.querySelector('video'); }
      function isAd() { return !!(document.querySelector('.ad-showing') || document.querySelector('.ad-interrupting')); }

      setInterval(function() {
        try {
          var v = getVideo();
          if (v) attach(v);
          var btn = document.querySelector('.ytp-ad-skip-button,.ytp-ad-skip-button-modern,.ytp-skip-ad-button,.ytp-ad-skip-button-slot');
          if (btn && btn.offsetParent !== null) { btn.click(); return; }
          if (!v) return;
          if (isAd() && isFinite(v.duration) && v.duration > 0) { v.currentTime = v.duration - 0.05; }
        } catch(e) {}
      }, 150);
      ...
      setInterval(function() {
        try {
          var v = getVideo();
          if (!v) return;
          if (v.paused && !window.__userPaused && !isAd() && v.readyState >= 3 && v.duration > 0) {
            v.play().catch(function(){});
          }
        } catch(e) {}
      }, 2000);
      ...
      function onNav() {
        ...
        if (!document.getElementById('__ytab')) {
          var s = document.createElement('style');
          s.id = '__ytab';
          s.textContent = 'ytm-promoted-video-renderer,ytm-ad-slot,[data-ad-slot-id]{display:none!important;}';
      ...
      try {
        ...
        history.pushState = function() { _pp.apply(history, arguments); setTimeout(onNav, 80); };
        history.replaceState = function() { _rr.apply(history, arguments); setTimeout(onNav, 80); };
        window.addEventListener('popstate', function() { setTimeout(onNav, 80); });
        document.addEventListener('yt-navigate-finish', function() { window.__userPaused = false; });
      } catch(e) {}
    })();
  """;
